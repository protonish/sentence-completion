...
cmd:argument('-pretext','the ssed text based on which the model predits the subsequent words')
-- optional parameters
cmd:option('-seed',99,'random number generator\'s seed')
cmd:option('-formatwords',1,'Insert appropriate spaces in text')
cmd:option('-sample',1,' 0 to use max at each timestep, 1 to sample at each timestep')
cmd:option('-primetext',"i declared",'used as a prompt to "seed" the state of the LSTM using a given sequence, before we sample.')
...
function predict_text(seed_text)
  seed_text = " "..seed_text.." "
  seed(seed_text)
  sentence = sample()
  sentence = seed_text..sentence
  return sentence
end

--local seed_text = "The common man " 
print(predict_text(opt.pretext))
